...
session_start();
	...
	if (isset($_POST)) {
		$tanaman = strtolower($_POST['tanaman']);
		$jenis = $_POST['jenis'];
		$suhu = $_POST['suhu'];
		$ph = $_POST['ph'];
		$kedalaman = $_POST['kedalaman'];
		$caritanaman = $db->query("SELECT * from jenis_tanaman where lower(tanaman) = '$tanaman'");
		// print_r("SELECT * from jenis_tanaman where lower(tanaman) = '$tanaman'");
		if ($caritanaman->num_rows) {
			$tanaman = $caritanaman->fetch_object()->idTanaman;
			...
			$_SESSION['flash'] = array("success"=>false,
								"msg"=>"Mohon maaf tanaman tersebut belum tersedia pada sistem kami",
								"data"=>$_POST);
			header("location:analisa.php");	
		}
	}else{
